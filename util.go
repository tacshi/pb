...
func formatBytes(i int64) (result string) {
	switch {
	case i >= _TiB:
		result = fmt.Sprintf("%.02f TB", float64(i)/_TiB)
	case i >= _GiB:
		result = fmt.Sprintf("%.02f GB", float64(i)/_GiB)
	case i >= _MiB:
		result = fmt.Sprintf("%.02f MB", float64(i)/_MiB)
	case i >= _KiB:
		result = fmt.Sprintf("%.02f KB", float64(i)/_KiB)
	default:
